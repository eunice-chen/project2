...
void Robot::SetSurroundingCell(FloorMap* map)
{
    ...
    // bottom
    if(currentCell->_position.x + 1 < map->trow &&
       (map->floor[currentCell->_position.x + 1][currentCell->_position.y]._pathState == BLANK || map->floor[currentCell->_position.x + 1][currentCell->_position.y]._pathState == TARGET )
        && map->floor[currentCell->_position.x + 1][currentCell->_position.y].distance == INT_MAX)
    {
    ...
    //top
    if(currentCell->_position.x - 1 >= 0 &&
       (map->floor[currentCell->_position.x - 1 ][currentCell->_position.y]._pathState == BLANK || map->floor[currentCell->_position.x - 1 ][currentCell->_position.y]._pathState == TARGET )
        && map->floor[currentCell->_position.x - 1 ][currentCell->_position.y].distance == INT_MAX)
    {
        // set its distance to currentDistance + 1
...
bool IsCleaned(Cell* c){return (c->state==clean);}

void Robot::CleanUpRemaining()
{
    remainingCells.remove_if(IsCleaned);
}
...
int main(int argc, char * argv[])
{
    ...
    outFile.open("final.path");
    {
        if(outFile.is_open())
        {
            ...
            {
                cout  << r->StepHistory.front().x << "   " << r->StepHistory.front().y << endl;
                r->StepHistory.erase(r->StepHistory.begin());
            }
        }
        outFile.close();
    }
